...
export default {
  ...
  created () {
    ...
    this.getTopYs = debounce(() => {
      this.topYs = []
      this.topYs.push(0)
      this.topYs.push(this.$refs.params.$el.offsetTop - 30)
      this.topYs.push(this.$refs.comment.$el.offsetTop)
      this.topYs.push(this.$refs.recommend.$el.offsetTop - 40)
  ...
  methods: {
    ...
    // 1.获取y值
      ...
      // 1.普通做法
        ...
        }else if(this.topYs[2] == undefined && positionY >= this.topYs[2]){
           this.currentIndex = 1
          this.$refs.nav.currentIndex = this.currentIndex
        }
      }
